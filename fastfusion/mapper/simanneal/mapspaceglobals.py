...
class MapspaceGlobals:
    def __init__(
        ...
        self.intermediate_tensor_names = set().union(*(self.workload.get_
        ...
        self.compatibility2leftcompatibility, self.compatibility2rightcompatibility, self.leftcompatibility2tiling, self.rightcompatibility2tiling = self._create_compatibility()
        self.size_scale = len(self.einsum2ranks)
        n_optimal = sum(
            len(s.mappings.data)
            for simlist in self.sims.values()
            for s in simlist
        )
        n_pmappings = sum(
    ...
    def _create_tensor2memories(self):
        tensor2memories = {}
        for t in self.intermediate_tensor_names:
            possible_memories = []
            for einsum_name, sim_list in self.sims.items():
                cur_memories = set()
                if t not in sim_list[0].tensor_names:
                    continue
                for sim in sim_list:
                    storage = sim.compatibility.get_reservation_by_name(t)
        ...
        for einsum_name, sim_list in self.sims.items():
            ...
            for sim in sim_list:
                for rank_index, loop in enumerate(sim.compatibility.loops):
                    x = einsum_rank_index_to_loops[einsum_name].setdefault(loop.rank_variable_name, {})
    ...
    def get_possible_translations(self, t: Compatibility, to_einsum: str):
        ...
        def translate_loop(l: Loop):
            compatible_ranks = set.union(
                *(full_equivalent_ranks[n] for n in l.rank_variable_names)
            ) & right_ranks
